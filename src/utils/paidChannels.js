...
const { colors, serverName, prices } = require('../config');
const { readRecord, writeRecord } = require('./database');

function buildGrantModal() {
  const modal = new ModalBuilder().setCustomId('pv_grant_modal').setTitle('Grant Paid Ad Channel');
  modal.addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('target_user_id').setLabel('Discord User ID').setStyle(TextInputStyle.Short).setPlaceholder('e.g. 123456789012345678').setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('access_days').setLabel('Access Duration (10 / 20 / 30 / 365 days)').setStyle(TextInputStyle.Short).setPlaceholder('10, 20, 30, or 365').setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('robux_paid').setLabel('Robux Amount Paid').setStyle(TextInputStyle.Short).setPlaceholder('e.g. 190').setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('slow_mode_reduction').setLabel('Slow Mode Reduction (0 or 30 minutes)').setStyle(TextInputStyle.Short).setPlaceholder('0 (none) or 30 (-60 Robux add-on)').setRequired(true)),
  );
  return modal;
}

function buildRevokeModal() {
  const modal = new ModalBuilder().setCustomId('pv_revoke_modal').setTitle('Revoke Paid Channel Access');
  modal.addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('target_user_id').setLabel('Discord User ID').setStyle(TextInputStyle.Short).setPlaceholder('e.g. 123456789012345678').setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('revoke_reason').setLabel('Reason for Revocation').setStyle(TextInputStyle.Short).setPlaceholder('e.g. Expired, Refunded, Violation').setRequired(true)),
  );
  return modal;
}
...
async function handleGrantModal(interaction, client) {
  ...
  try {
    ...
    return interaction.editReply({ content: `❌ Could not find user with ID \`${targetUserId}\`. Ensure they are in the server.` });
  }

  const slowModeSecs = Math.max(1800, 3600 - slowModeReduction * 60);
  const paidCatId    = cfg.channels.paidAdChannelsCategory;
  ...
  const paidChannel = await interaction.guild.channels.create({
    ...
    permissionOverwrites: [
      ...
      { id: targetMember.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory, PermissionFlagsBits.EmbedLinks, PermissionFlagsBits.AttachFiles] },
  ...
  await writeRecord(client, `PAIDCH:${targetMember.id}`, {
    ...
    accessDays, expiresAt, slowModeSecs, robuxPaid,
  });
  ...
  try {
    const pvCh = await client.channels.fetch(cfg.channels.paymentVerifierPanel);
    await pvCh.send({ embeds: [logEmbed] });
  } catch (_) {}

  await interaction.editReply({ content: `✅ Paid channel <#${paidChannel.id}> created for <@${targetMember.id}> — **${accessDays} days**.` });
}
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              ...
              `Open a ticket to renew at our current discounted prices:\n\n` +
              `~~${prices.days10.inflated} Robux~~ → **${prices.days10.real} Robux** — 10 Days  🏷️ 20% OFF\n` +
              `~~${prices.days20.inflated} Robux~~ → **${prices.days20.real} Robux** — 20 Days  🏷️ 20% OFF\n` +
              `~~${prices.days30.inflated} Robux~~ → **${prices.days30.real} Robux** — 30 Days  🏷️ 20% OFF\n` +
              `~~${prices.days365.inflated.toLocaleString()} Robux~~ → **${prices.days365.real.toLocaleString()} Robux** — 12 Months  🏷️ 20% OFF`
            )
            .setFooter({ text: `${serverName} • Premium Advertising` }),
        ],
      });
